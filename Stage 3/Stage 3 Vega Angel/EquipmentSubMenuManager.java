...
public class EquipmentSubMenuManager {
    ...
    public void createNewEquipment() {
        ...
        System.out.print("Do you want to create (1) Regular Equipment or (2) Special Equipment: ");
        int choice = Integer.parseInt(input.nextLine());

        System.out.print("Enter Name for Equipment: ");
        ...
        while (id == null) {
            System.out.print("Enter Id of Equipment (4 Integers): ");

            String idInput = input.nextLine().trim();

            if (idInput.isEmpty()) 
            {
                System.out.println("ERROR: Equipment ID cannot be empty.");
                continue;
            }
            ...
            try {
                ...
                System.out.println("ERROR: Equipment ID must be a number.");

            }
        }

        System.out.print("Enter date of purchase for Equipment: ");
        String datePurchased = input.nextLine().trim();

        System.out.print("Enter Average LifeSpan of Equipment in years: ");
        int avglife = Integer.parseInt(input.nextLine());

        Equipment exists = equipmentManager.searchEquipment(id);
        if (exists != null) 
        {
            System.out.println("ERROR: Equipment with this ID already exists.");
            return;
        }
        ...
        if (choice == 2) 
        {
            ...
            SpecializedEquipment seq =
            new SpecializedEquipment(name, id, datePurchased, avglife, startTime, endTime, date);
